// ABOUTME: Service for Istio VirtualService resource operations
// ABOUTME: Handles list, get, delete, stream, and manifest operations

import {
	BaseKubeService,
	type KubernetesListResponse,
} from "../base/BaseKubeService.js";
import { CRDNotInstalledError } from "../base/errors.js";
import type {
	VirtualServiceDetail,
	VirtualServiceHttpRoute,
	VirtualServiceListItem,
} from "./VirtualServiceService.types.js";

export class VirtualServiceService extends BaseKubeService {
	async listVirtualServices(
		namespace: string,
	): Promise<VirtualServiceListItem[]> {
		const crdExists = await this.checkCRDExists(
			"virtualservices.networking.istio.io",
		);
		if (!crdExists) {
			throw new CRDNotInstalledError(
				"virtualservices.networking.istio.io",
				"Istio VirtualService",
			);
		}

		return this.withCredentialRetry(async () => {
			const response =
				await this.customObjectsApi.listNamespacedCustomObject({
					group: "networking.istio.io",
					version: "v1beta1",
					namespace,
					plural: "virtualservices",
				});

			const list = response as unknown as KubernetesListResponse;
			return (list.items ?? []).map((item) =>
				this.mapVirtualServiceListItem(item),
			);
		});
	}

	async getVirtualService(
		namespace: string,
		name: string,
	): Promise<VirtualServiceDetail> {
		return this.withCredentialRetry(async () => {
			const response =
				await this.customObjectsApi.getNamespacedCustomObject({
					group: "networking.istio.io",
					version: "v1beta1",
					namespace,
					plural: "virtualservices",
					name,
				});

			return this.mapVirtualServiceDetail(
				response as Record<string, unknown>,
			);
		});
	}

	async getVirtualServiceManifest(
		namespace: string,
		name: string,
	): Promise<string> {
		return this.withCredentialRetry(async () => {
			const response =
				await this.customObjectsApi.getNamespacedCustomObject({
					group: "networking.istio.io",
					version: "v1beta1",
					namespace,
					plural: "virtualservices",
					name,
				});
			const manifest = this.sanitizeManifest(response);
			return JSON.stringify(manifest, null, 2);
		});
	}

	async deleteVirtualService(
		namespace: string,
		name: string,
	): Promise<void> {
		return this.withCredentialRetry(async () => {
			await this.customObjectsApi.deleteNamespacedCustomObject({
				group: "networking.istio.io",
				version: "v1beta1",
				namespace,
				plural: "virtualservices",
				name,
			});
		});
	}

	async streamVirtualServices(
		namespace: string,
		onData: (data: string) => void,
		onError: (err: unknown) => void,
		signal?: AbortSignal,
	) {
		const abortController = new AbortController();
		if (signal) {
			const handleAbort = () => abortController.abort(signal.reason);
			signal.addEventListener("abort", handleAbort, { once: true });
			abortController.signal.addEventListener("abort", () => {
				signal.removeEventListener("abort", handleAbort);
			});
		}

		let requestController: AbortController;

		const startWatch = async (
			retryOnAuth: boolean = true,
		): Promise<AbortController> => {
			try {
				return await this.watch.watch(
					`/apis/networking.istio.io/v1beta1/namespaces/${namespace}/virtualservices`,
					{},
					(type, obj) => {
						try {
							onData(
								JSON.stringify({
									type,
									object: this.mapVirtualServiceListItem(
										obj as Record<string, unknown>,
									),
								}),
							);
						} catch (err) {
							onError(err);
						}
					},
					(err) => {
						if (err && abortController.signal.aborted === false) {
							onError(err);
						}
					},
				);
			} catch (error) {
				const err = error as { statusCode?: number };
				if (err.statusCode === 401 && retryOnAuth) {
					this.refreshCredentials();
					return await startWatch(false);
				}
				throw error;
			}
		};

		try {
			requestController = await startWatch();
		} catch (error) {
			onError(error);
			abortController.abort();
			throw error;
		}

		abortController.signal.addEventListener("abort", () => {
			requestController.abort();
		});

		return () => {
			abortController.abort();
		};
	}

	private mapVirtualServiceListItem(
		resource: Record<string, unknown>,
	): VirtualServiceListItem {
		const metadata = resource.metadata as
			| Record<string, unknown>
			| undefined;
		const spec = resource.spec as Record<string, unknown> | undefined;

		const http = (spec?.http as Array<unknown>) ?? [];
		const tls = (spec?.tls as Array<unknown>) ?? [];
		const tcp = (spec?.tcp as Array<unknown>) ?? [];

		const creationTimestamp = metadata?.creationTimestamp
			? new Date(metadata.creationTimestamp as string).toISOString()
			: undefined;

		return {
			name: (metadata?.name as string) ?? "unknown",
			namespace: (metadata?.namespace as string) ?? "default",
			hosts: (spec?.hosts as string[]) ?? [],
			gateways: (spec?.gateways as string[]) ?? [],
			httpRouteCount: http.length,
			tlsRouteCount: tls.length,
			tcpRouteCount: tcp.length,
			creationTimestamp,
		};
	}

	private mapVirtualServiceDetail(
		resource: Record<string, unknown>,
	): VirtualServiceDetail {
		const listItem = this.mapVirtualServiceListItem(resource);
		const metadata = resource.metadata as
			| Record<string, unknown>
			| undefined;
		const spec = resource.spec as Record<string, unknown> | undefined;

		return {
			...listItem,
			labels: (metadata?.labels as Record<string, string>) ?? {},
			annotations: (metadata?.annotations as Record<string, string>) ?? {},
			http: (spec?.http as VirtualServiceHttpRoute[]) ?? [],
			tls: (spec?.tls as Array<Record<string, unknown>>) ?? [],
			tcp: (spec?.tcp as Array<Record<string, unknown>>) ?? [],
			exportTo: (spec?.exportTo as string[]) ?? [],
		};
	}
}
